...
from app.services.pricing import discount_failure_message, encode_payment_metadata, get_discount_preview, get_plan_price_toman, get_plan_price_usd, normalize_discount_code
    ...
    try:
        ...
        if group_id and topic_id:
            try:
                ...
                logging.getLogger("webapp").error(f"Failed to send ticket notification to group {group_id} topic {topic_id}: {e}")
    ...
    try:
        ...
        if group_id and topic_id:
            try:
                ...
                logging.getLogger("webapp").error(f"Failed to send ticket reply notification to group {group_id} topic {topic_id}: {e}")
...
async def webapp_recharge_payment(
    ...
    async with async_session_maker() as session:
        if plan_id is not None:
            ...
            plan_names = {
                1: "BNETS PRO (5GB) 1Month",
                2: "BNETS PRO (10GB) 1Month",
                3: "BNETS PRO (20GB) 1Month",
                4: "BNETS PRO (30GB) 1Month",
                5: "BNETS PRO (50GB) 1Month",
                6: "BNETS PRO (100GB) 1Month",
            }
            product_name = f"{service_type_label(payment_service_type)} - {plan_names.get(plan_id, f'BNETS PRO Plan {plan_id}')}"
            PLAN_GB_LIMITS = {1: 5, 2: 10, 3: 20, 4: 30, 5: 50, 6: 100}
            gb_limit = PLAN_GB_LIMITS.get(plan_id)
            volume = f"{gb_limit}GB" if gb_limit else "نامحدود"
        else:
            payable_amount = payment_amount
