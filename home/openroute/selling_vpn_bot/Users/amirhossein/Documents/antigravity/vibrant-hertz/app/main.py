...
async def lifespan(app: FastAPI):
    ...
    from app.db.models import Payment, SshServer
    try:
        async with async_session_maker() as session:
            ...
            # Reconcile historical payments whose server_id points at the wrong service pool.
            active_servers = (await session.scalars(select(SshServer).where(SshServer.status == "active"))).all()
            server_by_service_type = {}
            for server in active_servers:
                server_by_service_type.setdefault(server.service_type, server)

            payments = (await session.scalars(
                select(Payment).where(
                    Payment.server_id.is_not(None),
                    Payment.service_type.in_([ACCOUNT_TYPE_SSH, ACCOUNT_TYPE_V2RAY]),
                )
            )).all()

            updated_payments = 0
            for payment in payments:
                expected_server = server_by_service_type.get(payment.service_type)
                if expected_server and payment.server_id != expected_server.id:
                    payment.server_id = expected_server.id
                    updated_payments += 1

            if updated_payments:
                await session.commit()
                logger.info("Reconciled %s payment rows with corrected server_id/service_type mapping.", updated_payments)
    except Exception as e:
