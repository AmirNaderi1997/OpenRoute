...
import { CheckCircle, XCircle, Send, Users, Activity, DollarSign, Server, Plus, Power, PowerOff, ArrowLeft, RefreshCw, HardDrive, Search, ChevronRight, Megaphone, Globe } from 'lucide-react';
...
export const AdminManagement: React.FC = () => {
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'payments' | 'tickets' | 'broadcast' | 'seo'>('overview');
  ...
  const [seoActionLoading, setSeoActionLoading] = useState(false);
  ...
  const fetchUsers = async (searchStr = '', page = 1) => {
    ...
    try {
      const res = await apiClient.get(`/admin/webapp/users?page=${page}&limit=5000&search=${searchStr}`);
  ...
  const fetchSeoStatus = async () => {
    setSeoLoading(true);
    try {
      const res = await apiClient.get('/admin/webapp/seo');
      setSeoData(res.data);
    } catch (e) {
  ...
  const toggleSeoBot = async (enabled: boolean) => {
    setSeoActionLoading(true);
    try {
      await apiClient.post('/admin/webapp/seo/toggle', { enabled });
      setSeoData(prev => prev ? { ...prev, enabled } : null);
    } catch (e) {
      console.error("Failed to toggle SEO bot", e);
    } finally {
      setSeoActionLoading(false);
    }
  };

  const triggerSeoBot = async () => {
    setSeoActionLoading(true);
    try {
      await apiClient.post('/admin/webapp/seo/trigger');
      alert("سئو با موفقیت بازسازی و در سایت منتشر شد!");
      await fetchSeoStatus();
    } catch (e) {
      console.error("Failed to trigger SEO bot", e);
      alert("خطایی رخ داد.");
    } finally {
      setSeoActionLoading(false);
    }
  ...
  const handleSendBroadcast = async () => {
    ...
    try {
      const response = await apiClient.post('/admin/webapp/broadcast', { message: broadcastText });
      setBroadcastText('');
      alert(
        `ارسال پیام همگانی پایان یافت.\nموفق: ${response.data.success}\nناموفق: ${response.data.failed}`
      );
    } catch (err) {
    ...
    if (seoLoading && !seoData) {
      return (
        <div className="flex flex-col items-center justify-center py-20 space-y-4">
          <div className="w-10 h-10 border-t-2 border-r-2 border-indigo-500 rounded-full animate-spin" />
          <span className="text-xs text-gray-400">درحال بارگذاری تنظیمات سئو...</span>
        </div>
      );
    }

    return (
      <div className="space-y-6 max-w-md mx-auto animate-fade-in text-right" dir="rtl">
        {/* Header Section */}
        <div className="text-center">
          <h2 className="text-xl font-extrabold text-gradient-purple">تنظیمات ربات هوشمند سئو (SEO)</h2>
          <p className="text-xs text-gray-400 mt-1">مدیریت خودکار و هوشمند متاتگ‌های گوگل سایت</p>
        </div>

        {/* Auto Pilot Card */}
        <div className="glass-panel p-5 rounded-3xl border border-white/10 relative overflow-hidden shadow-xl">
          <div className="flex justify-between items-center">
            <div>
              <h3 className="font-extrabold text-sm text-white">خلبان خودکار سئو (Auto SEO Pilot)</h3>
              <p className="text-[10px] text-gray-400 mt-1">چرخش و بهینه‌سازی روزانه کلمات کلیدی در ساعت ۴ صبح</p>
            </div>
            <button
              onClick={() => toggleSeoBot(!seoData?.enabled)}
              disabled={seoActionLoading}
              className={`w-12 h-6 rounded-full p-1 transition-all duration-300 relative ${
                seoData?.enabled ? 'bg-indigo-600' : 'bg-slate-800 border border-white/10'
              }`}
            >
              <div className={`w-4 h-4 rounded-full bg-white transition-all duration-300 ${
                seoData?.enabled ? 'translate-x-6' : 'translate-x-0'
              }`} />
            </button>
          </div>

          <div className="mt-4 pt-4 border-t border-white/5 flex justify-between items-center text-[10px] text-gray-400">
            <span>آخرین بروزرسانی خودکار:</span>
            <span className="font-bold font-mono text-indigo-400">
              {seoData?.last_updated ? safeFormatDate(seoData.last_updated) : 'هنوز اجرا نشده'}
            </span>
          </div>
        </div>

        {/* Current Live Meta Data */}
        <div className="glass-panel p-5 rounded-3xl border border-white/10 space-y-4">
          <div className="flex items-center gap-1.5 text-indigo-400">
            <Globe size={14} />
            <h3 className="font-extrabold text-xs">متاتگ‌های فعال کنونی در سایت</h3>
          </div>
          
          <div className="space-y-3">
            <div>
              <span className="text-[9px] text-gray-400 block mb-1">عنوان صفحه (Meta Title):</span>
              <div className="bg-slate-900/50 p-3 rounded-xl border border-white/5 text-xs text-white leading-relaxed">
                {seoData?.title || '---'}
              </div>
            </div>
            
            <div>
              <span className="text-[9px] text-gray-400 block mb-1">توضیحات (Meta Description):</span>
              <div className="bg-slate-900/50 p-3 rounded-xl border border-white/5 text-xs text-white leading-relaxed font-sans">
                {seoData?.description || '---'}
              </div>
            </div>

            <div>
              <span className="text-[9px] text-gray-400 block mb-1">کلمات کلیدی (Meta Keywords):</span>
              <div className="flex flex-wrap gap-1.5 mt-1" dir="rtl">
                {seoData?.keywords ? seoData.keywords.split(', ').map((k, i) => (
                  <span key={i} className="bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2 py-0.5 rounded-md text-[9px] font-bold">
                    {k}
                  </span>
                )) : '---'}
              </div>
        ...
        {/* Action Panel */}
        <div className="glass-panel p-5 rounded-3xl border border-white/10 space-y-4">
          <h3 className="font-extrabold text-xs text-indigo-400">عملیات فوری</h3>
          <p className="text-[10px] text-gray-400 leading-relaxed">
            با کلیک روی دکمه زیر، ربات فوراً کلمات کلیدی، نام ماه‌های شمسی و تایتل سایت را تحلیل و بازسازی کرده و تغییرات را مستقیماً روی سرور اعمال می‌کند.
          </p>
          <button
            onClick={triggerSeoBot}
            disabled={seoActionLoading}
            className="w-full bg-gradient-premium text-white py-3.5 rounded-2xl font-bold text-xs transition-all active:scale-[0.98] shadow-lg shadow-indigo-600/30 flex items-center justify-center gap-1.5"
          >
            {seoActionLoading ? 'در حال بازسازی سئو...' : '🚀 بازسازی و انتشار آنی سئو'}
          </button>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-vazir pb-24 pt-4 px-4">
      ...
      <div className="flex bg-slate-900/40 p-1 rounded-2xl border border-white/5 backdrop-blur-xl max-w-md mx-auto mb-6 overflow-x-auto gap-1">
        <button 
          onClick={() => { setActiveTab('overview'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'overview' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          نظارت کل
        </button>
        <button 
          onClick={() => { setActiveTab('users'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'users' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          کاربران
        </button>
        <button 
          onClick={() => { setActiveTab('payments'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'payments' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          تراکنش‌ها
        </button>
        <button 
          onClick={() => { setActiveTab('tickets'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'tickets' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          تیکت‌ها
        </button>
        <button 
          onClick={() => { setActiveTab('broadcast'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'broadcast' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          اعلان
        </button>
        <button 
          onClick={() => { setActiveTab('seo'); setSelectedTicket(null); }}
          className={`flex-1 py-2 px-3 rounded-xl text-[10px] font-bold transition-all shrink-0 ${activeTab === 'seo' ? 'bg-gradient-premium text-white shadow-lg' : 'text-gray-400'}`}
        >
          سئو
        </button>
      </div>
