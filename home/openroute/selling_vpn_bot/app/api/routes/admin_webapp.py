...
async def process_broadcast(message_text: str) -> dict[str, int]:
    from app.db.database import async_session_maker
    ...
    for chat_id in all_chat_ids:
        try:
            ...
            try:
                ...
                logging.error(f"WebApp Broadcast retry failed for {chat_id}: {retry_exc}")
                fail_count += 1
            ...
            logging.exception(f"Unexpected WebApp Broadcast failure for {chat_id}: {e}")
            fail_count += 1
        await asyncio.sleep(0.05)

    return {
        "attempted": len(all_chat_ids),
        "success": success_count,
        "failed": fail_count,
    }

@router.post("/broadcast")
async def webapp_broadcast(req: BroadcastRequest) -> Dict[str, Any]:
    message_text = req.message.strip()
    if not message_text:
        raise HTTPException(status_code=400, detail="متن پیام خالی است")
    result = await process_broadcast(message_text)
    return {"status": "success", **result}

class SEOToggleRequest(BaseModel):
    enabled: bool
...
async def get_seo_status():
    import os
    import re
    from app.worker.seo_bot import load_seo_settings, INDEX_PATH
    ...
    if os.path.exists(INDEX_PATH):
        try:
            ...
            if not desc_match:
                desc_match = re.search(r'<meta\s+content="(.*?)"\s+name="description"\s*/?>', html, re.IGNORECASE)
            key_match = re.search(r'<meta\s+name="keywords"\s+content="(.*?)"\s*/?>', html, re.IGNORECASE)
            if not key_match:
                key_match = re.search(r'<meta\s+content="(.*?)"\s+name="keywords"\s*/?>', html, re.IGNORECASE)
                
            seo_data["title"] = title_match.group(1) if title_match else ""
            seo_data["description"] = desc_match.group(1) if desc_match else ""
            seo_data["keywords"] = key_match.group(1) if key_match else ""
        except Exception as e:
            logging.error(f"Failed to read index.html SEO tags: {e}")
            
    return seo_data

@router.post("/seo/toggle")
async def toggle_seo_bot(req: SEOToggleRequest):
    from app.worker.seo_bot import load_seo_settings, save_seo_settings
    settings_data = load_seo_settings()
    settings_data["enabled"] = req.enabled
    save_seo_settings(settings_data)
    return {"status": "success", "enabled": req.enabled}

@router.post("/seo/trigger")
async def trigger_seo_bot():
