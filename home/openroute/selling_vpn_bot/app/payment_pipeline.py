...
from app.services.pricing import decode_payment_metadata, mark_discount_code_as_used
    ...
    try:
        async with async_session_maker() as session:
            ...
            if payment.server_id:
                ...
                product_name = plan_names.get(plan_id, "BNETS PRO Plan")
                product_name = f"{service_type_label(payment.service_type)} - {product_name}"
...
async def _activate_service_for_payment(session, payment: Payment, user: User) -> tuple[bool, str]:
    ...
    success_text = (
        ...
        + Lexicon.purchase_success(account.ssh_username, account.ssh_password, import_link, str(connection["path"]))
    )
    try:
