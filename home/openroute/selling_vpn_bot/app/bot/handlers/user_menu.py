...
from app.db.models import User, SshAccount, SshServer, SupportTicket, TicketMessage
...
class SupportTicketFSM(StatesGroup):
    waiting_for_subject = State()
    waiting_for_message = State()


async def _is_support_admin(message: Message, user: User | None) -> bool:
...
async def _notify_support_ticket(message: Message, ticket_id: int, subject: str, text: str) -> None:
    import html
    import logging
    ...
    user_display = (
        ...
        else f"<code>{message.from_user.id}</code> (بدون نام کاربری)"
    )
    reply_markup = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text="✍️ پاسخ به تیکت",
            url=f"https://t.me/{settings.BOT_USERNAME}?start=reply_ticket_{ticket_id}",
        )
    ]])
    alert_text = (
        f"🎫 <b>تیکت جدید (ربات)</b>\n\n"
    ...
    if not group_id:
        logging.getLogger("user_menu").error(
            "Cannot notify support: manager group is not configured for ticket %s",
            ticket_id,
        )
        return
    ...
    try:
        if topic_id:
            try:
                ...
                logging.getLogger("user_menu").warning(
                    "Ticket topic delivery failed; retrying manager group: %s", exc
                )
        ...
        logging.getLogger("user_menu").exception(
            "Failed to notify manager group about ticket %s", ticket_id
        )
...
async def cmd_s_recommend_start(message: Message):
    await message.reply("⚠️ دستور ناپایدار یا نامشخص.\n\n💡 لطفاً از دستور <b>/start</b> برای ورود به منوی اصلی استفاده کنید.")
...
async def cmd_start(message: Message, state: FSMContext, command: CommandObject = None):
    ...
    if args and args.startswith("reply_ticket_"):
        ...
        async with async_session_maker() as session:
            ...
            is_admin = await _is_support_admin(message, user)
...
async def show_support(callback: CallbackQuery, state: FSMContext):
    await state.set_state(SupportTicketFSM.waiting_for_subject)
...
@router.message(
    StateFilter(SupportTicketFSM.waiting_for_subject, SupportTicketFSM.waiting_for_message),
    Command("cancel"),
)
async def cancel_support_ticket(message: Message, state: FSMContext):
    await state.clear()
...
@router.message(SupportTicketFSM.waiting_for_subject, F.text)
async def receive_support_subject(message: Message, state: FSMContext):
    subject = message.text.strip()
    if not 3 <= len(subject) <= 120:
        await message.answer("موضوع تیکت باید بین ۳ تا ۱۲۰ کاراکتر باشد.")
        return

    await state.update_data(subject=subject)
    await state.set_state(SupportTicketFSM.waiting_for_message)
    await message.answer(
        "لطفاً متن کامل پیام خود را ارسال کنید.\nبرای انصراف /cancel را بفرستید."
    )


@router.message(SupportTicketFSM.waiting_for_message, F.text)
async def receive_support_message(message: Message, state: FSMContext):
    text = message.text.strip()
    if not 1 <= len(text) <= 4000:
        await message.answer("متن پیام باید بین ۱ تا ۴۰۰۰ کاراکتر باشد.")
        return

    data = await state.get_data()
    subject = str(data.get("subject", "پشتیبانی"))
    ...
    async with async_session_maker() as session:
        ...
        session.add(ticket)
        await session.flush()
        session.add(TicketMessage(ticket_id=ticket.id, sender="user", text=text))
        await session.commit()
        ticket_id = ticket.id

    await state.clear()
    await _notify_support_ticket(message, ticket_id, subject, text)
    await message.answer(
        f"✅ پیام شما با شماره تیکت <b>#{ticket_id}</b> برای پشتیبانی ارسال شد.",
        reply_markup=get_main_menu(),
    )
...
async def buy_step_pay(callback: CallbackQuery, callback_data: BuyCallback, state: FSMContext):
    ...
    if discount_code:
        ...
        if not preview["discount_applied"]:
            ...
            builder.button(text="🪙 پرداخت با رمز ارز", callback_data=BuyCallback(step="pay_select_crypto", server_id=server_id, plan_id=plan_id))
            ...
            await callback.message.edit_text(
                ...
                "لطفاً کارت به کارت یا پرداخت رمز ارز را انتخاب کنید.",
                reply_markup=builder.as_markup(),
            )
            return
