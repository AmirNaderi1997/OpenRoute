...
async def notify_admins_of_pending_payment(payment_id: int):
    try:
        async with async_session_maker() as session:
            ...
            if group_id and topic_id:
                try:
                    ...
                    try:
                        ...
                        logger.error(f"Fallback to general group failed: {e2}")
            elif group_id:
                try:
                    await _send_payment_notification(group_id, msg, kb, file_id, is_doc)
                except Exception as e:
            ...
            # Send to all admins' private chats as requested
            admins = (await session.scalars(select(User).where(User.is_admin == True))).all()
            for admin in admins:
                try:
                    await _send_payment_notification(admin.id, msg, kb, file_id, is_doc)
                except Exception as e:
                    logger.error(f"Failed to notify admin {admin.id} privately: {e}")
