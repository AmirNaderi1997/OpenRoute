...
async def show_connections(message: Message):
    ...
    if not servers:
        await status_msg.edit_text("هیچ سروری ثبت نشده است.")
        return
    ...
    for srv in servers:
        try:
            ...
            # Find Dropbear or SSHD active established sessions
            cmd = "ps -ef | grep -E 'sshd: .*@notty|dropbear.*[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+' | grep -v grep | wc -l"
            out = await ssh._run_command(srv.ip_address, cmd)
            count = int(out.strip()) if out and out.strip().isdigit() else 0
            
            report += f"🖥 **{srv.name}** ({srv.ip_address}): {count} اتصال فعال\n"
            total_connections += count
        except Exception as e:
            report += f"🖥 **{srv.name}**: خطا در برقراری ارتباط با سرور\n"
...
async def sync_traffic(message: Message):
    ...
    from app.worker.scheduler import check_expired_accounts
